...
import gameConfigJson from './russian-oblasts.json'
...
describeOrSkip('Russian Federal Subjects Game Configuration', () => {
  ...
  it('should have exactly 83 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(83)
  })

  it('should have 83 total features', () => {
    expect(processedData.features).toHaveLength(83)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('russian-oblasts')
    expect(gameConfig.name).toBe('Russian Federal Subjects')
