...
import gameConfigJson from './uk-counties.json'
...
describeOrSkip('UK Counties & Regions Game Configuration', () => {
  ...
  it('should have exactly 192 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(192)
  })

  it('should have 192 total features', () => {
    expect(processedData.features).toHaveLength(192)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('uk-counties')
    expect(gameConfig.name).toBe('UK Counties & Regions')
