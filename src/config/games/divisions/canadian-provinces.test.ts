...
import gameConfigJson from './canadian-provinces.json'
...
describeOrSkip('Canadian Provinces & Territories Game Configuration', () => {
  ...
  it('should have exactly 13 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(13)
  })

  it('should have 13 total features', () => {
    expect(processedData.features).toHaveLength(13)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('canadian-provinces')
    expect(gameConfig.name).toBe('Canadian Provinces & Territories')
