...
import gameConfigJson from './paris-districts.json'
...
describeOrSkip('Paris Quartiers d\'Usage Game Configuration', () => {
  ...
  it('should have exactly 44 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(44)
  })

  it('should have 44 total features', () => {
    expect(processedData.features).toHaveLength(44)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('paris-districts')
    expect(gameConfig.name).toBe('Paris Quartiers d\'Usage')
