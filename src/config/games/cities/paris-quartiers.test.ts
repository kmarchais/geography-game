...
import gameConfigJson from './paris-quartiers.json'
...
describeOrSkip('Paris Quartiers Administratifs Game Configuration', () => {
  ...
  it('should have exactly 80 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(80)
  })

  it('should have 80 total features', () => {
    expect(processedData.features).toHaveLength(80)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('paris-quartiers')
    expect(gameConfig.name).toBe('Paris Quartiers Administratifs')
