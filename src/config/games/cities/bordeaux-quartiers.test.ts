...
import gameConfigJson from './bordeaux-quartiers.json'
...
describeOrSkip('Bordeaux Area Quartiers Game Configuration', () => {
  ...
  it('should have exactly 75 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(75)
  })

  it('should have 75 total features', () => {
    expect(processedData.features).toHaveLength(75)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('bordeaux-quartiers')
    expect(gameConfig.name).toBe('Bordeaux Area Quartiers')
