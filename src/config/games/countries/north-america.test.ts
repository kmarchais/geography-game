...
import gameConfigJson from './north-america.json'
...
describeOrSkip('North American Countries Game Configuration', () => {
  ...
  it('should have exactly 38 unique territories', () => {
    ...
    expect(uniqueEntities).toHaveLength(38)
  })

  it('should have 38 total features', () => {
    expect(processedData.features).toHaveLength(38)
  })
  ...
  it('should have correct game metadata', () => {
    expect(gameConfig.id).toBe('north-america-countries')
    expect(gameConfig.name).toBe('North American Countries')
