// Update the template to show the correct total rounds
  const updateGameInfoDisplay = () => {
    if (availableDepartments.value.length > 0) {
      // Log the total count of territories
      console.log(`Game will use ${totalRoundsLocal.value} out of ${availableDepartments.value.length} total territories`);
    }
  };

  watch(totalRoundsLocal, updateGameInfoDisplay);// Import the specific Control class from Leaflet for better TypeScript support
import { Control } from 'leaflet';.territory-marker {
  background: none;
  border: none;
}
...
.territory-icon {
  ...
  cursor: pointer;
            ...
            leafletMapInstance.setView([46.78, -56.17], 10); // Saint Pierre and Miquelon
            break;
          case 'polynesia':
            leafletMapInstance.setView([-17.68, -149.45], 9); // French Polynesia (centered on Tahiti)
            break;
          case 'newcaledonia':
            leafletMapInstance.setView([-21.25, 165.5], 8); // New Caledonia
            break;
          case 'antarctic':
            leafletMapInstance.setView([-49.35, 70.22], 7); // French Southern and Antarctic Lands (Kerguelen Islands)
            break;          case 'all':
            leafletMapInstance.setView([20, 0], 2); // World view
            break;@media screen and (max-width: 768px) {
  .game-header {
    top: auto;
    bottom: 20px;
    width: 90%;
